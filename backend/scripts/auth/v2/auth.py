...
ALGORITHM = "HS256"
ACCESS_TOKEN_EXPIRE_HOURS = 1  # Access Token 1시간 유효 (UMS-AUT-04)
REFRESH_TOKEN_EXPIRE_DAYS = 7  # Refresh Token 7일 유효 (UMS-AUT-04)
...
def create_refresh_token(user_data: dict) -> str:
    ...
    작업내용 3번: Refresh Token (7일 유효) 발급
    """
    token_data = {
...
# 4. 인증 미들웨어
# ============================================
async def auth_middleware_v2(request: Request):
    """
    토큰 검증 미들웨어 V2
    UMS-AUT-02: 권한 분리 (admin/client)
    """
    # Authorization 헤더 확인
    ...
    print(f"사용자 인증 성공: {user_info.get('email')} (role: {user_info.get('role')})")
...
async def require_role(request: Request, allowed_roles: list):
    """
    특정 역할만 접근 가능하도록 제한
    UMS-AUT-02: Admin은 모든 기능 접근 가능, Client는 자사 영상만
    """
    ...
    if user_role not in allowed_roles:
        raise HTTPException(
            status_code=403,
            detail=f"접근 권한이 없습니다. 필요 권한: {', '.join(allowed_roles)}"
        )
