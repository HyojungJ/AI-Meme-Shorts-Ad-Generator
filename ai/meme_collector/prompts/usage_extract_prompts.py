USAGE_EXTRACT_PROMPT_V1 = """웹 검색 결과에서 밈의 **실제 활용 사례**를 추출하세요.

## 입력 형식
각 검색 결과는 다음 형식입니다:
```
--- 출처: https://example.com/page ---
제목
본문 내용...
```

## 핵심 규칙
1. 검색 결과에서 **모든 활용 사례**를 추출하세요
2. **source_url 필수**: 각 사례를 발견한 검색 결과의 "--- 출처: URL ---"에서 URL을 그대로 복사하세요
3. 구체적인 사례를 그대로 인용 - 일반화하지 마세요
4. 검색 결과에 없는 내용은 절대 생성하지 마세요
...
## 출력 예시
context: "네이버 블로그에서 제품 홍보에 활용"
usage: "'운동 많이 된다~' 형식으로 제품 효과 강조"
tone: "playful"
example_type: "good"
source_url: "https://blog.naver.com/example/123"  ← 반드시 포함!

## 찾아야 할 사례 유형
- 브랜드/기업 마케팅, 아이돌/연예인 홍보, 기관/단체 홍보
- 일상 패러디, 챌린지/따라하기

**검색 결과에 활용 사례가 없으면 빈 배열 []을 반환하세요.**"""


# V1->V2
# 국가인권위원회 '혐오표현 판단기준에 관한 토론회' 참고함.
USAGE_EXTRACT_PROMPT_V2 = """웹 검색 결과에서 밈의 **실제 활용 사례**를 추출하세요.
